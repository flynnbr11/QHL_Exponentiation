...
def exp_ham(src, t, plus_or_minus = -1.0, precision=1e-18, scalar_cutoff = 25, print_method=False):
    import numpy as np
    n_qubits=np.log2(np.shape(src)[0])
    if n_qubits >= 7.0:
      return exp_ham_sparse(src, t, plus_or_minus = -1.0, precision=precision, scalar_cutoff=scalar_cutoff, print_method=print_method)
    else: 
      return exponentiate_ham(src, t, plus_or_minus = -1.0, precision=precision, scalar_cutoff=scalar_cutoff, print_method=print_method)


def exp_ham_trotter(src, t, plus_or_minus = -1.0, precision=1e-18, scalar_cutoff = 25, print_method=False, trotterize_by=1.0):
    import numpy as np
    ...
    if n_qubits >= 7:
      exp_iHt = exp_ham_sparse(src, t, plus_or_minus = -1.0, precision=precision, scalar_cutoff=scalar_cutoff, print_method=print_method)
